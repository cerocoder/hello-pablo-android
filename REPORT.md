...
| Плюсы | Минусы |
|---|---|
| Один job вместо двух — сборка и тест в одном месте, короче итоговый лог run'а | До зелёного CI дошёл дольше и болезненнее Kotlin: минимум 3 красных прогона против 1 (нехватка диска → неудачный первый фикс диска, стёрший сам Flutter SDK → два красных прогона подряд из-за построчного `script:`) |
...
| `android/` не пришлось писать с нуля вручную построчно — только сверить с официальным шаблоном | `android/`-обвязку нельзя сгенерировать штатной командой `flutter create` без локального Flutter SDK — пришлось воспроизводить шаблон вручную |
...
| Красных прогонов до зелёного | 1 (конфликт AGP 9 с плагином `org.jetbrains.kotlin.android`) | ≥3 (нехватка диска, стёртый диском фиксом Flutter SDK, дважды построчный `script:`) |
