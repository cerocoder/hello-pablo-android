...
6. Запушил той же командой (`GIT_SSH_COMMAND=... git push origin
   HEAD:main`); коммит `d4e88ac`.
7. **Первый прогон CI (`d4e88ac`) упал** с «No space left on device» на
   шаге установки system-image эмулятора
...
8. **Ловушка внутри ловушки — второй красный прогон (`9ebfb15`).**
   Первая версия этого фикса использовала `tool-cache: true` — а эта
   опция чистит `$AGENT_TOOLSDIRECTORY` (`/opt/hostedtoolcache`), ровно
   туда `subosito/flutter-action` с `cache: true` кладёт сам Flutter SDK.
   Этот прогон удалил Flutter и упал с `exit 127`. Финальная
   конфигурация: `tool-cache: false`, `android: false` (сам Android SDK
   нужен), остальное (`dotnet`, `haskell`, `large-packages`,
   `docker-images`, `swap-storage`) — `true`. У Kotlin-тикета этой
   проблемы не было — предположительно из-за меньшего суммарного объёма
   зависимостей (`gradle/actions/setup-gradle` легче, чем полный Flutter
   SDK + pub cache).
9. **Следующий прогон (`007424d`) — зелёный.** Это была первая приёмка
   тикета 02: на этот момент обе технологии впервые были зелёными
   одновременно.
10. **Ревью тикета 02 нашло замечания** (тестовый отчёт как отдельный
    артефакт, более честная формулировка комментария, `paths:`,
    `permissions:`) — правки внёс коммитом `6aa13cc`. Этот коммит
    переформатировал `script:` в несколько физических строк и тем самым
    **внёс регрессию, которой раньше не было**: `reactivecircus
    /android-emulator-runner` выполняет **каждую физическую строку YAML
    отдельным вызовом `/usr/bin/sh -c`** — переменные (PID логката, код
    выхода теста) не переживают переход между строками. Прогон `6aa13cc`
    упал: сам тест на эмуляторе реально прошёл («All tests passed!») —
    падение было чисто в обвязке: `kill "$LOGCAT_PID"` и `exit
    "$TEST_EXIT_CODE"` получали пустую строку → «Illegal number» → шаг
    падал с exit code 2, хотя тест был зелёным.
11. Починил тем, что переписал весь `script:` **одной физической строкой**
    (коммит `2c36bd2`), без `| tee` (раннер использует `dash`, там нет
    `PIPESTATUS`, пайп теряет реальный exit-код) — вывод редиректится в
    файл через `>`, код забирается через `$?` сразу после, файл `cat`-ится
    для видимости в логе CI, и только потом `exit`. Прочитать лог
    упавшего шага (`6aa13cc`) через GitHub API без токена было нельзя (то
    же ограничение, что и в Kotlin-тикете) — причину нашёл по логу,
    который вручную скопировал пользователь. Прогон `2c36bd2` —
    финальный зелёный, один джоб `build-and-test`, все 13 реальных шагов
    зелёные.
...
| Плюсы | Минусы |
|---|---|
| Один job вместо двух — сборка и тест в одном месте, короче итоговый лог run'а | Дорога до финального зелёного была не линейной: 3 красных прогона против 1 у Kotlin (нехватка диска; неудачный первый фикс диска, стёрший сам Flutter SDK; регрессия от собственного фикса по итогам код-ревью) — причём тикет уже был зелёным один раз (`007424d`, первая приёмка), прежде чем следующий коммит временно сломал именно CI-обвязку |
...
| Красных прогонов до финального зелёного | 1 (конфликт AGP 9 с плагином `org.jetbrains.kotlin.android`), сразу за ним — финальный зелёный | 3 (нехватка диска → фикс диска стёр Flutter SDK → регрессия от фикса по код-ревью), с одним зелёным прогоном между ними (`007424d` — первая приёмка тикета 02) |
...
живой SSH-отладки — но разной ценой и разной формой пути: у Kotlin один
точный, предсказуемый фикс (устаревший плагин, явная ошибка от самого
Gradle) и сразу финальный зелёный. У Flutter путь был не линейным: два
красных прогона вокруг диска, затем первая приёмка тикета 02 на зелёном
`007424d` — и тут же следующий коммит, вносивший чисто косметические
правки по итогам код-ревью (тестовый отчёт, честный комментарий, `paths:`,
`permissions:`), неожиданно сломал именно CI-обвязку регрессией в
`script:`, потребовавшей ещё одного прогона (`2c36bd2`), чтобы снова
стать зелёным. Это нормальная часть процесса: сам код-ревью и
последующая правка тоже могут временно сломать уже рабочий CI, если
затрагивают инфраструктурный YAML, а не код приложения. Ни один из
снэгов в обоих случаях не был связан с самим экраном `"Hello Pablo"` —
оба приложения по коду тривиальны и заработали с первой попытки; вся
боль была в обвязке CI/эмулятора, что и есть честный ответ на вопрос
«насколько сложна эмуляция и отладка без локальных инструментов»: сама
эмуляция через `reactivecircus/android-emulator-runner` в обоих случаях
несложная, а вот доводка CI-окружения вокруг неё требует итераций — но
полностью решаема без единого локального инструмента, что и было целью.
